...
# Update the remote user in the om_config.sh file
echo "Updating remote user in om_config.sh to $remote_user"
...
awk -v user="$remote_user" '
    ...
    {print}
' "$script_dir/om_config.sh" > "$script_dir/om_config.sh.tmp" && 
mv "$script_dir/om_config.sh.tmp" "$script_dir/om_config.sh"
